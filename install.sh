#!/usr/bin/env bash
# Install jasonv-skills for one or more agents:  ./install.sh [claude|codex|cursor|cline|all]
# Each agent reads a native per-skill dir; we symlink each skill into it. Override any dir with
# CLAUDE_SKILLS_DIR / CODEX_SKILLS_DIR / CURSOR_SKILLS_DIR / CLINE_SKILLS_DIR.
...
case " ${agents[*]} " in *" all "*) agents=(claude codex cursor cline);; esac
...
for a in "${agents[@]}"; do case "$a" in
  ...
  cline)  link_into "${CLINE_SKILLS_DIR:-$HOME/.cline/skills}"   cline;;
  *) echo "unknown agent: $a (use claude|codex|cursor|cline|all)" >&2; exit 2;;
esac; done
